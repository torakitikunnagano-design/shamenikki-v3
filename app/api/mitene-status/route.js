...
export async function POST(request) {
  try {
    ...
    const { data: castRows, error: castErr } = await supabase
      ...
      .limit(1);
    if (castErr) {
      console.error("[mitene-status] supabase error:", castErr.message);
    ...
    if (!vpsRes.ok) {
      console.error("[mitene-status] VPS error", vpsRes.status, resText);
    }

    return NextResponse.json(data, { status: vpsRes.status });
  } catch (e) {
    console.error("[mitene-status] proxy error:", e.message, e.stack);
