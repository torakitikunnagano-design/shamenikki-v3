...
const VPS_URL = "http://160.251.166.73:3000/bulk-mitene/acquire";

export async function POST(request) {
  try {
    const { shopdir } = await request.json();

    const payload = { shopdir };
    ...
    if (!vpsRes.ok) {
      console.error("[bulk-mitene-acquire] VPS error", vpsRes.status, resText);
    }

    return NextResponse.json(data, { status: vpsRes.status });
  } catch (e) {
    console.error("[bulk-mitene-acquire] proxy error:", e.message, e.stack);
