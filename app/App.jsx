...
const DEFAULT_STORES = [
  ...
  { id: "lj_brothers",             name: "LJブラザーズ" },
  { id: "club_audition_saku",      name: "クラブオーディション佐久" },
  { id: "club_audition_matsumoto", name: "クラブオーディション松本" },
  { id: "viking",                  name: "バイキング" },
];
  ...
  try {
    ...
    if (raw) {
      ...
      if (Array.isArray(arr) && arr.length) {
        // 保存済みリストはそのまま（順序・内容を保持）、DEFAULT_STORES の未収録idだけ後ろに追加
        const have = new Set(arr.map((s) => s.id));
        const merged = [...arr, ...DEFAULT_STORES.filter((s) => !have.has(s.id))];
        if (merged.length !== arr.length) {
          try { localStorage.setItem(STORES_KEY, JSON.stringify(merged)); } catch {}
        }
        return merged;
      }
    }
...
{"sessions":[{"courseMin":60,"shimei":"本指名","courseFee":5000,"shimeiRyou":2000,"extCount":0,"extMin":0,"extFee":0,"op":0,"subtotal":7000}],"gross":50000,"misc":3000,"dorm":10000,"transport":1000,"takeHome":36000}`;
  ...
  async function saveRecord() {
    ...
    try {
      ...
      // 再保存時は approved=false に戻して再び承認待ちにする。image_path は含めず既存写真を消さない。
      const { error: stmtErr } = await supabase.from("salary_statements").upsert(
        { store_id: getActiveStoreId(), cast_id: castId, date, approved: false, approved_at: null, reject_reason: null, rejected_at: null, staff_resolved: false, staff_resolved_at: null, uploaded_at: new Date().toISOString() },
                          ...
                          <div style={{ display: "grid", gap: "4px" }}>
                            {bd.sessions.map((s2, j) => {
                              ...
                              <div key={j} style={{ display: "grid", gap: "2px" }}>
                                ...
                                <p style={{ fontSize: "12px", fontWeight: "800", color: C.text, margin: 0, lineHeight: 1.5, whiteSpace: "nowrap" }}>小計{yen(sub)}円</p>
                              </div>
                              );
                            })}
                          </div>
